...
def _pred(game_pk=1, away="TB", home="NYY", away_pid=100, home_pid=200):
    return pd.DataFrame([{
...
def test_finds_hot_batter_vs_home_pitcher():
    ...
    out = compute_notable_bvp_matchups(pred, top5, bvp)
    assert len(out) == 1
...
def test_respects_n_limit():
    ...
    bvp = _bvp([{"batter": i, "pitcher": 200, "pa": 15 + i, "obp": 0.500, "avg": 0.400, "hr": 1}
               for i in range(1, 6)])
    out = compute_notable_bvp_matchups(pred, top5, bvp, n=2)
    assert len(out) == 2


def test_empty_inputs_return_empty_list():
    assert compute_notable_bvp_matchups(pd.DataFrame(), pd.DataFrame(), pd.DataFrame()) == []
